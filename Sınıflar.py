class UAV:
    def __init__(self, irtifa, hiz_x=0, hiz_y=0, hiz_z=0):
        self.hiz_x = hiz_x
        self.hiz_y = hiz_y
        self.hiz_z = hiz_z
        self.irtifa = irtifa
    def anlik_hiz(self):
        return (self.hiz_x**2 + self.hiz_y**2 + self.hiz_z**2)**0.5
...
class Cevre:
    def __init__(self, ruzgar_Vx=0, ruzgar_Vy=0, ruzgar_Vz=0, yercekimi_ivmesi=9.81, hava_yogunlugu=1.225):
        self.ruzgar_Vx = ruzgar_Vx
        self.ruzgar_Vy = ruzgar_Vy
        self.ruzgar_Vz = ruzgar_Vz
        self.yercekimi_ivmesi = yercekimi_ivmesi
        self.hava_yogunlugu = hava_yogunlugu
    def anlik_ruzgar(self):
        return (self.ruzgar_Vx**2 + self.ruzgar_Vy**2 + self.ruzgar_Vz**2)**0.5
...
class BalistikHesap:
    ...
    def dusme_hesapla(self, uav, yuk, cevre):
        ...
        dt = 0.1
        while y > 0:
            toplam_hiz = (vx**2 + vy**2 + vz**2)**0.5
            surunme_kuvveti = 0.5 * rho * toplam_hiz**2 * Cd * yuk_alan
            
            x_ivme = - (surunme_kuvveti/yuk_agirlik) * (vx/toplam_hiz)
            y_ivme = -g - (surunme_kuvveti/yuk_agirlik)  * (vy/toplam_hiz)
            z_ivme = - (surunme_kuvveti/yuk_agirlik)  * (vz/toplam_hiz)
